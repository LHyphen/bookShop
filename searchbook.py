        ...
        while True:
            if len(isbn) is 0 and len(author) is not 0:
                sql = "select book.ISBN,BookName,Author,Price,TotalNum " \
                      "from book left join collectionofbook c on book.ISBN = c.ISBN where author = '%s' " % str(author)
                break
            elif len(isbn) is not 0 and len(author) is 0:
                sql = "select book.ISBN,BookName,Author,Price,TotalNum " \
                      " from book left join collectionofbook c on book.ISBN = c.ISBN where book.ISBN = '%s' " % str(isbn)
                break
            elif len(isbn) is not 0 and len(author) is not 0:
                sql = "select book.ISBN,BookName,Author,Price,TotalNum " \
                " from book left join collectionofbook c on book.ISBN = c.ISBN " \
                      "where book.ISBN = '%s' and author = '%s' " % (str(isbn), str(author))
